...
**Codex: the provider refuses several `env_vars` names when the sandbox holds a workspace-owner credential** — that is, under `inference_auth: "sandbox"` **or** `keep_workspace_pat: true`. The codex-runtime set is `CODEX_PATH`, `CODEX_HOME`, `CODEX_CONFIG`, `MODEL_PROVIDER`, `DEFAULT_AUTH_REQUEST`, `INITIAL_AGENT_MODE`, `CODEX_API_KEY`, `OPENAI_API_KEY`, `APP_SERVER_LOGS`; on top of that, every runtime refuses `DATABRICKS_HOST`/`DATABRICKS_TOKEN`, proxy and CA-bundle variables, and the loader classes (`NODE_*`, `LD_*`, `DYLD_*`, `npm_config_*`, `PYTHON*`, `GIT_*`). Each can redirect where the baked owner credential is sent, or load code into the process holding it. Under `inference_auth: "env"` they are all permitted — the token is then your own — but `CODEX_PATH` will still break the runtime outright: the adapter spawns it as `<path> app-server`, and the image's `/usr/local/bin/codex` is a Databricks `ucode` wrapper that accepts no arguments. Leave it unset and the adapter resolves its own bundled binary directly, with no `PATH` lookup at all.
