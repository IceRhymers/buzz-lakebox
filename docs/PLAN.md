...
- Install verification: post-extract, run `buzz-agent --version` (or equivalent no-op invocation — exact flag confirmed at M0.5) in-sandbox; deploy fails if the runtime can't execute.
...
7. **Secret handover**: render the env file — `BUZZ_PRIVATE_KEY`, `BUZZ_AUTH_TAG`, `BUZZ_RELAY_URL`, `BUZZ_ACP_AGENT_COMMAND/ARGS`, `BUZZ_ACP_AGENTS` (parallelism), `BUZZ_ACP_SYSTEM_PROMPT`, `BUZZ_ACP_MODEL`, `BUZZ_ACP_RESPOND_TO(_ALLOWLIST)`, the three timeout vars, `NOSTR_PRIVATE_KEY` + `GIT_CONFIG_*` (git-credential-nostr, lane A §4), plus merged `env_vars` — and ship it **over SSH stdin only, never argv**, into `$HOME/.buzz-backend/env` with `umask 077` (README op table; lane C gap #3 states the requirement). **The stdin transport itself is unprobed** (critique #6): lane C only verified argv exec. M0.5 probes `echo test | databricks sandbox ssh <id> -- 'cat > /tmp/x && sha256sum /tmp/x'`. Named fallback if the CLI doesn't forward stdin: raw `ssh` using the CLI-generated ProxyCommand config (`sandbox config`, verified in lane C), which forwards stdin natively and stays off argv; chosen at M0.5, before M1 code depends on it.
...
10. **Verify, then set autostop policy**: success signal = `pgrep -f buzz-acp` still alive after N seconds (N tuned at M0.5; log-tailing is **best-effort diagnostics only** — buzz-acp's log vocabulary is unknown until the M0.5 probe, critique #7). Only after verification: `sandbox config <id> --no-autostop` or `--idle-timeout <provider_config.idle_timeout>` (1m–24h, lane C). Default per Open Decision 3, informed by the M0.5 WSS-vs-idle probe.
...
**M0.5 — Probe session (one live sitting on tanner-west, before dependent decisions)** (critique #6, #7, #8)
- SSH-stdin passthrough: `echo test | databricks sandbox ssh <id> -- 'cat > /tmp/x && sha256sum /tmp/x'` → selects step-7 transport or its named fallback.
- WSS-vs-idle: hold a WSS connection open from in-sandbox Node past the 10-min default idle window → does the sandbox autostop? Feeds Open Decision 3.
- buzz-acp behavior with a non-member throwaway key: does it stay up, exit, or crash-loop on 403 `relay_membership_required`? Capture actual log vocabulary → defines M1's observable pass signal and step-10's N.
- `buzz-agent` no-op invocation for runtime verification (§4.2); boot-hook survey (cron/user-systemd in the microVM) for the optional M3 supervisor — informational only, nothing depends on it.
- **Accept**: a probe-results note answering all four, appended to `docs/` (same style as lane C).
...
- **Accept** (all observable, per M0.5-calibrated signals; throwaway nsec only):
  1. Fresh deploy → `{ok:true, agent_id}` < 180 s; sandbox named `buzz-<npub12>-<slug>` exists; binaries + `buzz-agent` runnable in `$HOME`; env file 0600; in-sandbox `databricks current-user me` **fails** (PAT reset); buzz-acp state matches the M0.5-documented on-403 behavior, with its actual logged relay-contact line present in `acp.log`.
...
**M2 — Lifecycle**
- `status` (sandbox state + in-sandbox pgrep + one-shot log tail + CLI version), `stop`, `start` (start → wait Running → `launch.sh`, PAT stub re-asserted), `logs` (one-shot), `undeploy` (shred → delete).
- **Accept**: `stop` → Stopped, process dead; `start` → Running and buzz-acp process group present ≤ 60 s without re-running deploy, and `current-user me` still fails in-sandbox; `undeploy` → sandbox gone from `sandbox list`; `status` emits stable JSON.

**M3 — Hardening**
- `keep_workspace_pat` opt-out flag + acceptance test; redaction audit (marker-secret fuzz across all error paths); double-launch guard proof; error taxonomy (every failure mode → distinct actionable `ok:false`); optional in-sandbox supervisor **only if** M0.5's boot-hook survey found a real hook; operator runbook (autostop/lifetime-cap recovery, `!shutdown` interplay — lane A §6, Beta-storage-loss playbook).
- **Accept**: deploy with `keep_workspace_pat:true` → in-sandbox `current-user me` succeeds as creator; marker-secret fuzz shows zero leakage in stdout/stderr/logs; deploy-twice yields exactly one process group; fresh-machine runbook walkthrough succeeds.
...
| SSH-stdin transport unproven | critique #6 | M0.5 probe before M1; named fallbacks staying off argv |
| Autostopped/lifetime-capped/Beta-wiped sandbox = silently dead agent; v1 has no status op | lane C persistence, gaps #1/#2 | Honest framing: silent death is possible in **any** configuration; relay observer frames are the only health signal (lane A §6). `start` subcommand is the recovery path; mention replay bounds damage (lane A §5); default per Open Decision 3 after the M0.5 probe |
...
## 9. OPEN DECISIONS (owner-level)

1. **nsec trust boundary — confirm the stance.** Shipping the agent's nsec from the owner's keyring into Databricks-managed Beta storage is the premise of this provider (README; lane C proof note b). Confirm: acceptable for per-agent minted keys, rotation-on-suspicion as remedy? (§5 assumes yes.)
2. **Baked creator PAT default.** Proposed: reset it (agent cannot act as the owner on the workspace); `keep_workspace_pat: true` opt-in for owners who *want* a workspace-capable agent. Pure capability-vs-blast-radius call.
3. **Compute posture default — corrected framing.** The agent can go silently dead in *any* configuration (autostop is only one of three causes alongside Beta storage deletion and lifetime cap — lane C; relay observer frames are the only health signal — lane A §6). The setting therefore trades a 24/7 4vCPU/8GiB per-agent bill on the owner's workspace (rate not probed — owner to price) against eliminating exactly one silent-death cause. If the M0.5 WSS-vs-idle probe shows a connected harness never idles out, the default idle-timeout is free reliability and `--no-autostop` buys nothing; decide after that probe.
4. **v0 supported runtime set.** Proposed: `buzz-agent` only (ships in the .deb, zero extra provisioning), goose/claude/codex rejected with a clear error until v0.1 (npm adapters + config bridges + API-key handling). Owner-visible consequence: which of their agents can be deployed to Databricks at all in v0.

*(Engineering-level open questions inlined where they arise: §3.1 exact min CLI version — M0; §4.2 buzz-agent verification invocation — M0.5; §4.4 step 5 version pinning is by design since the payload carries no version — §0; §4.4 step 6 persona/env verification — M0 source pass.)*
