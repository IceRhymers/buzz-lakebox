...
**M3 — Hardening**
- `keep_workspace_pat` opt-out flag + acceptance test; redaction audit (marker-secret fuzz across all error paths); double-launch guard proof; error taxonomy (every failure mode → distinct actionable `ok:false`); ~~optional in-sandbox supervisor~~ **struck** *(M0.5: no boot hooks exist — lane D §5; provider `start` is the only recovery path)*; operator runbook (autostop/lifetime-cap recovery, `!shutdown` interplay — lane A §6, Beta-storage-loss playbook).
