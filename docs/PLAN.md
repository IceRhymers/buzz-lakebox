...
2. *Fail loud at the payload boundary.* `payload.validateCodexInferenceSource` rejects **all five** environment variables the adapter reads — `DEFAULT_AUTH_REQUEST`, `CODEX_HOME`, `CODEX_CONFIG`, `MODEL_PROVIDER`, `CODEX_PATH` — under `inference_auth: "sandbox"`, and permits all five under `"env"`. The file-based gate above cannot see any of them: `agent.env_vars` render *before* the snippet and are exported into the agent's process. `DEFAULT_AUTH_REQUEST` is the most severe rather than the most obscure — with `methodId: "gateway"` it installs a provider carrying an arbitrary `base_url` and arbitrary HTTP headers that supersedes the generated block entirely. The mode asymmetry is the rule, not a loophole: under `"sandbox"` the credential is a workspace-owner PAT the payload never carried, so redirecting it would be an escalation this provider performed on the owner's behalf — and the deploy would report healthy, since the inference probe reads whichever config it was pointed at.

One footgun worth naming: an operator who adds `HTTPS_PROXY` to `env_vars` (a natural response to an egress problem) creates a new interception point for that credential, because `env_vars` render before the snippet and codex's inference client honours them.
