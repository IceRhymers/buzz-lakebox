...
- No goose/codex runtime support (§4.2, Open Decision 4). Payloads requesting them are rejected with an actionable error, not half-deployed. **Claude Code is supported as of v0.1** (issue #2).
...
- **Supported runtimes.** Payload validation (step 1 below, before any provisioning) rejects an `agent_command` outside this table with an actionable error naming the alternatives and issue #3.
