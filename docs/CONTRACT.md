...
| `agent_command` | string | one of `buzz-agent`, `claude-agent-acp` (aliases `claude-code-acp`, `claude-code`, `claudecode`), `codex-acp` (aliases `codex`, `codex-cli`), else reject (PLAN §4.2). Bare `claude` is deliberately NOT accepted: it names the underlying CLI, not the ACP adapter. Bare `codex` IS accepted and canonicalized to `codex-acp` — upstream's zero-arg identity list contains it, and canonicalization means the image's `ucode` wrapper is never spawned |
...
Validated by the desktop before it reaches us (`validate_provider_config`, backend.rs:390-421): flat object, ≤20 fields, ≤64 KB, scalar values only, no secret-like key names (`secret|password|token|key|credential` as word segments). Our keys (`profile`, `idle_timeout`, `keep_workspace_pat`, `buzz_version`, `inference_auth`, `claude_adapter_version`, `codex_adapter_version`) all pass — `inference_auth`'s own segments (`inference`, `auth`) don't match the filter, which is exactly why the knob is named that and never `*token*`.

- `inference_auth` — string enum `""` \| `"env"` \| `"sandbox"`, default `""` (treated as `"env"`). `"env"`: current behavior — the provider resets the sandbox's baked creator-identity PAT to a stub and the owner supplies `DATABRICKS_HOST`/`DATABRICKS_TOKEN` via `agent.env_vars`. `"sandbox"`: zero-token — the provider skips the PAT reset/re-assert, keeps the baked `~/.databrickscfg`, and derives `DATABRICKS_HOST`/`DATABRICKS_TOKEN` from it at every launch. **Derivation is all-or-nothing, and `agent.env_vars` may not supply either one in this mode** (rejected at validation): deriving only one half let a payload pair its own endpoint with the sandbox's owner-level token. The same mode also refuses `env_vars` names that redirect or intercept that credential, or load code into the process holding it — proxy/CA variables and the `NODE_*`/`LD_*`/`DYLD_*`/`npm_config_*`/`PYTHON*`/`GIT_*` loader classes, plus the codex-runtime set (PLAN §5). `"sandbox"` supersedes `keep_workspace_pat` when both are set on the same deploy — they're redundant, not conflicting: both keep the baked cfg in place, and both trigger the `env_vars` restrictions above.
